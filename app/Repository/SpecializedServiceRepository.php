...
class SpecializedServiceRepository implements CargoEcommerce
{
    public function getAllData()
    {
        return $specialized_service = SpecializedService::first();
    }
    ...
    {
        $specialized_service = SpecializedService::find($id);

        if(isset($data['pro_sourcing_img'])){
            if ($specialized_service->pro_sourcing_img && file_exists(public_path('specialized_service/'.$specialized_service->pro_sourcing_img))){
                unlink(public_path('specialized_service/'.$specialized_service->pro_sourcing_img));
            }
            $imgname_1 = mt_rand(10000, 99999). '.' . $data['pro_sourcing_img']->getClientOriginalExtension();
            $data['pro_sourcing_img']->move('specialized_service/', $imgname_1);
        }
        else{
            $imgname_1 = $specialized_service->pro_sourcing_img;
        }

        if(isset($data['free_warehousing_img'])){
            if ($specialized_service->free_warehousing_img && file_exists(public_path('specialized_service/'.$specialized_service->free_warehousing_img))){
                unlink(public_path('specialized_service/'.$specialized_service->free_warehousing_img));
            }
            $imgname_2 = mt_rand(10000, 99999). '.' . $data['free_warehousing_img']->getClientOriginalExtension();
            $data['free_warehousing_img']->move('specialized_service/', $imgname_2);
        }
        else{
            $imgname_2 = $specialized_service->free_warehousing_img;
        }

        if(isset($data['inspection_img'])){
            if ($specialized_service->inspection_img && file_exists(public_path('specialized_service/'.$specialized_service->inspection_img))){
                unlink(public_path('specialized_service/'.$specialized_service->inspection_img));
            }
            $imgname_3 = mt_rand(10000, 99999). '.' . $data['inspection_img']->getClientOriginalExtension();
            $data['inspection_img']->move('specialized_service/', $imgname_3);
        }
        else{
            $imgname_3 = $specialized_service->inspection_img;
        }

        if(isset($data['repacking_img'])){
            if ($specialized_service->repacking_img && file_exists(public_path('specialized_service/'.$specialized_service->repacking_img))){
                unlink(public_path('specialized_service/'.$specialized_service->repacking_img));
            }
            $imgname_4 = mt_rand(10000, 99999). '.' . $data['repacking_img']->getClientOriginalExtension();
            $data['repacking_img']->move('specialized_service/', $imgname_4);
        }
        else{
            $imgname_4 = $specialized_service->repacking_img;
        }

        if(isset($data['custom_clr_img'])){
            if ($specialized_service->custom_clr_img && file_exists(public_path('specialized_service/'.$specialized_service->custom_clr_img))){
                unlink(public_path('specialized_service/'.$specialized_service->custom_clr_img));
            }
            $imgname_5 = mt_rand(10000, 99999). '.' . $data['custom_clr_img']->getClientOriginalExtension();
            $data['custom_clr_img']->move('specialized_service/', $imgname_5);
        }
        else{
            $imgname_5 = $specialized_service->custom_clr_img;
        }

        if(isset($data['d_step_delivery_img'])){
            if ($specialized_service->d_step_delivery_img && file_exists(public_path('specialized_service/'.$specialized_service->d_step_delivery_img))){
                unlink(public_path('specialized_service/'.$specialized_service->d_step_delivery_img));
            }
            $imgname_6 = mt_rand(10000, 99999). '.' . $data['d_step_delivery_img']->getClientOriginalExtension();
            $data['d_step_delivery_img']->move('specialized_service/', $imgname_6);
        }
        else{
            $imgname_6 = $specialized_service->d_step_delivery_img;
        }

        $specialized_service->update([
    ...
    {
        return $specialized_service = SpecializedService::find($id);
    }
